...
def test_not_equl_validator_custom_msg(spark_session):
    ...
    validation_map = {"trn_typ": [NotEqualValidator(['C', 'D'], custom_msg_lmda=custom_msg_lmda)], }
...
def test_not_equl_validator_custom_msg_multi_columns(spark_session):
    ...
    invaild_df_expected = spark_session.createDataFrame(
        [[3, "Invaild", None, "Column trn_typ has value: expected values are: ['C', 'D']|Column trn_desc has value:Invaild expected values are: ['Credit', 'Debit']|"]], \
        "trn_id: int, trn_desc: string, trn_typ:String, exception_desc:String")
...
def test_not_equl_validator_custom_msg_multi_clm_cust_clm_name(spark_session):
    ...
    data_val = val_builder.add_excp_rec_handler(excep_record_handler).add_config(config).add_validation_map(validation_map).add_excp_clm_provider(
        excp_msg_clm_provider).add_validate_rec_df(test_df).build()
    valid_df, invalid_df = data_val.validate()
...
def test_not_equl_validator_null_column(spark_session):
    validation_map = {"trn_typ": [NotEqualValidator(['C', 'D'])], }
