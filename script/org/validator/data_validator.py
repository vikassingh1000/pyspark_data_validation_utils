...
class DataValidator(IDataValidator):
    ...
    def validate(self):
        ...
        p_df_to_validate = self._validate_rec(self.df_to_validate,
                                              self.clmn_to_validate_against)

        p_df_to_validate.cache()

        after_validation_clms = p_df_to_validate.schema.names

        li_dif = [i for i in after_validation_clms + initial_clms if
                  i in after_validation_clms and i not in initial_clms]
        self.logger.info(li_dif)
        excp_rec_cond = None

        for clm in li_dif:
            if excp_rec_cond is None:
                excp_rec_cond = (~is_clm_null(clm, self.confg) & (
                            sf.col(clm) != self.sep))
            else:
                excp_rec_cond = excp_rec_cond | (
                            ~is_clm_null(clm, self.confg) & (
                                sf.col(clm) != self.sep))
        ...
        if self.exception_rec_handler:
            df_with_invalid_rec = self.exception_rec_handler.handle(
                df_with_invalid_rec)
    ...
    def _validate_rec(self, p_df_to_validate, p_clmn_to_validator):
        self.logger.info(
          f"Starting Validating records column using given validators :{p_clmn_to_validator}")

        if self.confg.excp_msg_agg:
            self.logger.info(
                f"Exception message would be aggregated so, create column with name {self.excp_clm_nm} and value {self.sep} ")
            p_df_to_validate = p_df_to_validate.withColumn(self.excp_clm_nm,
                                                           sf.lit(""))

        for l_clm, l_validator in p_clmn_to_validator.items():
            p_df_to_validate = self._validate_clm(l_validator, p_df_to_validate,
                                                  l_clm)
    ...
    def _validate_clm(self, p_validator, p_df_to_validate, p_clm):
        self.logger.info( f"Validating column {p_clm} with {p_validator}")

        if isinstance(p_validator, list):
            for l_val in p_validator:
                p_df_to_validate = self.call_validator(l_val, p_clm,
                                                       p_df_to_validate)
        elif isinstance(p_validator, Validator):
            self.call_validator(p_validator, p_clm, p_df_to_validate)
        else:
            raise Exception(f"Validator could be list of  Validators or single validator, however received: {p_validator} ")
...
class ValidatorBuilder(ABC):
    ...
    def add_excp_rec_handler(self, excep_rec_handler):
        ...
        if not isinstance(excep_rec_handler, IExceptionRecordHandler):
            raise Exception(
                "Exception handler should implement IExceptionRecordHandler")
...
class DefaultExceptionRecordHandler(IExceptionRecordHandler):
    """
    Default exception record handler, it will simple return the DF, one can pass there own implementation for it
    """
    logger = logging.getLogger('example_logger')

    def handle(self, exception_rec_df):
        self.logger.info(
            "Handling  exception records, this is default behaviour it will simple ignore the dataframe")
