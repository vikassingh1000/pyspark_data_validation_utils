...
class Validator(ABC):
    ...
    def _add_exeception_msg(self,p_df_to_validate, p_clm,validate_against):
        try:
            ...
            return  p_df_to_validate.withColumn(exp_clm_name, sf.when(compare_function(p_clm, validate_against),custom_msg_lmda(p_clm, validate_against)).otherwise(default_val_for_valid_rec))
    ...
    def _clm_sticher(self, excp_clm_name, *col):
        ...
        if ExceptionValidatorContext.confg.excp_msg_agg:
            return sf.concat(sf.col(excp_clm_name), *col, sf.lit(self.sep))
        else:
